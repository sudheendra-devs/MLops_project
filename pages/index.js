...
<p style={{ color: '#666' }}>This site automatically wakes all Hugging Face Spaces listed in your environment variables.</p>
...
<div style={{ marginTop: 18 }}>
<strong>Overall:</strong> {loading ? 'Warming up — demos may take 30–60s' : 'All demos running ✅'}
</div>


<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12, marginTop: 18 }}>
{statuses.map(s => (
<div key={s.space} style={{ border: '1px solid #e6e6e6', borderRadius: 8, padding: 12 }}>
<div style={{ fontWeight: 600 }}>{s.space}</div>
<div style={{ color: '#444' }}>{s.stage}</div>
{s.stage === 'RUNNING' ? (
<a href={`https://huggingface.co/spaces/${s.space}`} target='_blank' rel='noreferrer'>Open Demo</a>
) : (
